...
def load_audio_mono(path: Path, target_sr: int = SR) -> torch.Tensor:
    """Load an audio file as mono float32 without relying on torchaudio codecs."""
    path = Path(path)
    if sf is not None:
    ...
    if sr != target_sr:
        try:
            ...
            raise ModuleNotFoundError("Install librosa to resample non-16k audio.") from exc
        audio_np = librosa.resample(audio_np, orig_sr=sr, target_sr=target_sr).astype(np.float32)
